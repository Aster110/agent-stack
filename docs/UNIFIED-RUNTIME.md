...
Channel input: stable channel-local message ID, configured channel name, verified endpoint and text. Runtime derives namespaced ID and transport source; clients cannot select another thread. Optional `images` carries absolute paths of transport-validated local images; they reach the engine as native `localImage` input of the same turn after the text and are persisted in the WAL `fetched` entry, so replay keeps them. A vanished file is named in the text instead; if the engine refuses the image input the turn is retried once as text only. The WeChat transport supports text, voice transcripts, photos and files: media is downloaded from the WeChat CDN, AES-128-ECB decrypted, checked (magic-byte MIME, dimensions, size, file md5/length), stored content-addressed in `stateRoot/wechat-media` (0700/0600, TTL `wechat.mediaTtlHours`, default 72) and rendered in original item order; failures deliver the words with an explicit error marker after bounded retries. Video and untranscribed voice are named, not downloaded. Mesh messages whose relay materialized `meta.attachments[].localPath` become `[attachment n]` lines plus native images. Acceptance returns accepted/duplicate only after WAL fsync and durable handoff tombstone. Failure throws, retaining raw intake. The tombstone survives the existing 1,000-ID recent cache and WAL compaction.
